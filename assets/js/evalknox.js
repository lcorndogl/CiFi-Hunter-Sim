function EVALKNOX(lvl, maxStage, hp, atk, regen, dr, block, effect, charge, chargeGain, reload, proj, revival, calyp, ua, ghost, omen, ll, pog, finish, kraken, amp, dead, sear, pirate, timeless, torpedos, charger, armory, elixer, reflect, gadget, iters, iap, special, ultima, glac, quartz, tess, reviveCd, respec, bossLootRate, iterative, glacRate1, quartzRate1, tessRate1, xpRate1, hp1, atk1, regen1, dr1, block1, effect1, charge1, chargeGain1, reload1, proj1, gadget1, lvl1, time1) {
    var multi = (enemyNum) => Math.max(1, 1 + (enemyNum - 49) * .006 + Math.max(0, (enemyNum - 99) * .006) + Math.max(0, (enemyNum - 149) * .006) + Math.max(0, (enemyNum - 199) * .006) + Math.max(0, (enemyNum - 249) * .006) + Math.max(0, (enemyNum - 299) * .006))
    ...
    var sim = (knox) => {
        ...
        var regen = () => {
            ...
            if (knox.charger) {
                knox.charge1 += knox.charger * .02
                knox.charge2 += knox.charger * .02
                knox.totalCharge += knox.charger * .02
            }
        ...
        var torpedo = () => {
            for (var i = 0; i < 5 + knox.torpedos; i++) {
                currentEnemy.hp -= knox.currentAtk * 30 * (1 + .08 * knox.charger) * (1 + .2 * knox.torpedos) * currentEnemy.dr;
                if (currentEnemy.hp <= 0)
                    killEnemy()
                else {
        ...
        var loopLoot = normalized / 10 * ((Math.pow(1.074, Math.floor(enem / 10)) - 1) / .074 * 10 + (enem - Math.floor(enem / 10) * 10) * Math.pow(1.074, Math.floor(Math.min(enem, 1000) / 10))) * (1 + knox.timeless * .13) * (1 + knox.ll * .2 * knox.effect) * gadgetLootMulti;
        if (enem > 1000) {
